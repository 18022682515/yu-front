...
function resolver(transform){
    let obj = {};
    let { translate,translateX,translateY, scale,scaleX,scaleY, skew,skewX,skewY, rotate } = transform;
    Object.keys(transform).forEach(key=>{
        if(/translate/.test(key)){
            obj.translate = translate || [ translateX||0,translateY||0 ];
        }else if(/scale/.test(key)){
            obj.scale = scale || [ scaleX||1,scaleY||1 ];
        }else if(/skew/.test(key)){
            obj.skew = skew || [ skewX||0,skewY||0 ];
        }else if(/rotate/.test(key)){
            obj.rotate = rotate && [ rotate ];
        }
    });
    return obj;
}

export function getMatrix(transform){
    transform = resolver(transform);
    let all = [];
    for(let attr in transform){
        let first = attr[0].toLocaleUpperCase();
        let fn = obj['get'+first+attr.slice(1)];
        let arr = transform[attr];
        fn && all.push( fn(...arr) );
    }
